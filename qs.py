...
if __name__ == '__main__':  
    ...
    t1 = timeit.timeit(lambda: selu(x,alpha=1.67,lmbda=1.05).block_until_ready(),timer=time.perf_counter,number=1,)
    print(x.devices())
    ...
    _ = selu_jit(x) 
    t2 = timeit.timeit(lambda: selu_jit(x,alpha=1.67,lmbda=1.05).block_until_ready(),timer=time.perf_counter,number=1,)
